...
const opcionesClientes = ['Cliente 1', 'Cliente 2', 'Cliente 3'];
const opcionesVendedores = ['Vendedor 1', 'Vendedor 2', 'Vendedor 3'];
const opcionesProductos = ['Gasohol 95', 'Diesel B5', 'Gasohol 98 Premium'];
const opcionesTurnos = ['Mañana', 'Tarde', 'Noche'];
			...
			<section className="bg-[#1F2937] p-4 rounded-lg mb-6 shadow-lg">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end mb-4">
					<div>
						...
						<input
							type="date"
							className="w-full bg-gray-700 text-gray-200 border border-gray-600 rounded-md p-2"
						/>
					</div>
					<div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
					<div>
						<label className="block text-sm text-[#6B7280] mb-1">Cliente</label>
						<select
							multiple
							className="w-full bg-gray-700 text-gray-200 border border-gray-600 rounded-md p-2 h-24"
						>
					...
					<div>
						<label className="block text-sm text-[#6B7280] mb-1">Vendedor</label>
						<select
							multiple
							className="w-full bg-gray-700 text-gray-200 border border-gray-600 rounded-md p-2 h-24"
						>
					...
					<div>
						<label className="block text-sm text-[#6B7280] mb-1">Producto</label>
						<select
							multiple
							className="w-full bg-gray-700 text-gray-200 border border-gray-600 rounded-md p-2 h-24"
						>
							...
							<option>Gasohol 98 Premium</option>
						</select>
					</div>
					<div>
						<label className="block text-sm text-[#6B7280] mb-1">Turno</label>
						<select
							multiple
							className="w-full bg-gray-700 text-gray-200 border border-gray-600 rounded-md p-2 h-24"
						>
			...
			<section className="bg-[#111827] rounded-lg shadow-lg overflow-hidden">
							...
							{datosDeReporte.map((item, i) => {
								...
								return (
									<tr key={i} className="hover:bg-[#1F2937] transition">
										...
										<td className="p-4">
											<span
												className={`px-3 py-1 text-xs rounded-full ${estilos.badge}`}
											>
												{item.tipo}
											</span>
										</td>
				...
				<div className="p-4 bg-[#1F2937] flex justify-between items-center">
					...
					<div className="flex space-x-1">
						<button className="px-3 py-1 border border-[#F97316] bg-[#F97316] rounded-md text-sm text-white">
							1
						</button>
					</div>
				</div>
